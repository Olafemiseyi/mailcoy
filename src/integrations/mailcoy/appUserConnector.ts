...
// Server-only. Reads LOVABLE_API_KEY + the connector client API key
// from process.env to call the Lovable connector gateway.
...
export async function callAsAppUser({
  ...
  return fetch(`${gatewayBaseUrl}/${connectorId}${p}`, { ...init, headers });
}
