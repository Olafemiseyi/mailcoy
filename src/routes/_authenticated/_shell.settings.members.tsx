  ...
  return (
    <div className="space-y-6 max-w-3xl">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-line pb-5">
          <div>
            <h3 className="font-display text-lg font-bold text-ink">
              Team Admins & Dashboard Access
            </h3>
            <p className="mt-1 text-[13px] text-ink-3">
              People who have administrative login access to manage this Mailcoy workspace, domains,
              and billing.
            </p>
          </div>
          <Button
      ...
      {openModal && (
        ...
        >
          <div className="w-full max-w-md rounded-2xl border border-line bg-surface p-4 sm:p-6 shadow-xl relative max-h-[90vh] overflow-y-auto overflow-x-hidden">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 font-display text-[16px] sm:text-lg font-bold text-ink min-w-0">
              ...
              <button
                ...
                className="p-1.5 text-ink-3 hover:text-ink hover:bg-ink/[0.04] rounded-lg transition shrink-0"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
              ...
              <div className="mt-4 space-y-4">
                <div className="rounded-xl border border-line bg-surface-muted/30 p-3.5">
                  <div className="flex items-start gap-2.5">
                    <div className="rounded-full bg-green-500/10 p-1 mt-0.5 shrink-0">
                      <Check className="h-4 w-4 text-green-600" />
                    </div>
                    <p className="text-[13px] leading-relaxed text-ink-2 break-words">
                      {result.message}
                    </p>
                  </div>
                </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="h-10 flex-1 min-w-0 rounded-xl border border-line bg-surface-muted px-3 text-[12px] font-mono text-ink outline-none"
                      />
                    ...
                    <p className="text-[11.5px] text-ink-4">
                      Send this link to the user to let them sign up and join your workspace.
                    </p>
                    ...
                    <div className="flex flex-wrap items-center gap-2 pt-1">
                      <button
                        ...
                        onClick={() =>
                          window.open(
                            `https://wa.me/?text=${encodeURIComponent("Join my Mailcoy workspace! Here is your invite link:\n" + result.inviteUrl)}`,
                            "_blank",
                          )
                        }
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          (window.location.href = `mailto:?subject=${encodeURIComponent("Invitation to join Mailcoy workspace")}&body=${encodeURIComponent("Hello,\n\nYou have been invited to join my workspace on Mailcoy.\n\nPlease click the link below to sign up and join:\n\n" + result.inviteUrl + "\n\nBest,")}`)
                        }
                        className="flex items-center gap-2 h-8 px-3 rounded-lg border border-line bg-surface hover:bg-surface-muted transition text-[12px] font-medium text-ink-2"
                      >
                ...
                <Button className="w-full mt-2" onClick={() => setOpenModal(false)}>
                  Done
                </Button>
              </div>
            ) : (
              <form onSubmit={handleInvite} className="space-y-4 pt-3">
                <label className="grid gap-1.5 text-[13px] font-semibold text-ink-2">
                  ...
                  <input
                    ...
                    className="h-10 w-full min-w-0 rounded-xl border border-line bg-surface px-3 text-[13px] outline-none transition focus:border-primary"
                  />
                </label>
                ...
                <label className="grid gap-1.5 text-[13px] font-semibold text-ink-2">
                  ...
                  <CustomSelect
                    options={[
                      { value: "admin", label: "Admin (DNS, Inboxes, Signatures)" },
                      { value: "member", label: "Member (Staff Access)" },
                    ]}
                    value={role}
