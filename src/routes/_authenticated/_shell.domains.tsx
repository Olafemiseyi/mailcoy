...
import { PageHeader, Card, Button, Input, Field, StatusPill, ConfirmDeleteModal } from "@/components/app/AppShell";
import { Plus, RefreshCw, ChevronRight, Trash2, Globe, HelpCircle, Sparkles, CheckCircle2, ArrowUpRight, X, MessageSquare, Search, AlertCircle } from "lucide-react";
  ...
  pendingComponent: () => (
    <div className="space-y-6 pb-16">
      <div className="flex justify-between items-start gap-4">
        <div className="space-y-2 w-full max-w-sm"><Skeleton className="h-8 w-40" /><Skeleton className="h-4 w-full" /></div>
        <Skeleton className="h-10 w-32" />
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="border border-line rounded-2xl p-5 space-y-3">
            <div className="flex justify-between items-center"><Skeleton className="h-5 w-40" /><Skeleton className="h-6 w-20" /></div>
  ...
  return (
    <div>
      ...
      <Card className="p-6 mb-6">
        <form onSubmit={onAdd} className="flex flex-col md:flex-row gap-3 md:items-end">
        ...
        <div className="mt-3.5 pt-3 border-t border-line flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <button
            type="button"
            onClick={() => { setShowGuide(true); setRdapResult(null); }}
          ...
          >
            ...
            <span>Don't have a domain yet? Click here for free RDAP availability check & concierge options &rarr;</span>
          </button>
        </div>
      ...
      {showGuide && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 animate-in fade-in duration-150">
          <div className="relative w-full max-w-xl rounded-2xl border border-line bg-surface p-6 shadow-2xl space-y-5 max-h-[90vh] overflow-y-auto">
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-display text-[17px] font-bold text-ink">Need a domain for your business?</h3>
                  <p className="text-[12.5px] text-ink-3">Check live ICANN availability or let our team handle setup.</p>
                </div>
              </div>
              <button
            ...
            <div className="rounded-xl border border-line bg-background p-4 space-y-3">
              <span className="text-[11px] font-bold uppercase tracking-wider text-ink-4 block">
                ⚡ Instant ICANN RDAP Domain Check
              </span>
              <form onSubmit={handleRdapCheck} className="flex gap-2">
                <input
                  type="text"
                  value={checkDomainInput}
                  onChange={(e) => { setCheckDomainInput(e.target.value); setRdapError(null); }}
                ...
                />
                <Button type="submit" disabled={checkingRdap || !checkDomainInput.trim()} className="h-9 px-3 text-[12.5px]">
              ...
              {rdapError && (
                <div className="flex items-center gap-2 text-[12px] text-amber-700 dark:text-amber-400 bg-amber-500/10 border border-amber-500/30 rounded-lg px-3 py-2">
              ...
              {rdapResult && (
                <div className={`p-3 rounded-lg border text-[12.5px] animate-in fade-in duration-150 ${
                  ...
                  {!rdapResult.isRegistered ? (
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        ...
                        <p className="mt-0.5 text-[11.5px] opacity-90">You can register this domain now below or request concierge setup.</p>
                      </div>
                    </div>
                  ) : (
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <strong>🔒 {rdapResult.domain} is already registered</strong>
                        {rdapResult.registrarName && <span className="block text-[11.5px] text-ink-3">Registrar: {rdapResult.registrarName}</span>}
                        <p className="mt-1 text-[11.5px] text-ink-3">If you own this domain, you can add it directly to Mailcoy!</p>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {/* Option 1: Self-Service in 2 minutes */}
            <div className="rounded-xl border border-line bg-background p-4 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-bold uppercase tracking-wider text-emerald-600 bg-emerald-500/10 px-2 py-0.5 rounded-full">
                  Option A · Self-Service (2 mins)
                </span>
                <span className="text-[12px] font-mono text-ink-3">~{currency === "NGN" ? "₦12,000 / yr" : "$10 - $12 / yr"}</span>
              </div>
              <p className="text-[13px] text-ink-2 leading-relaxed">
                Register a domain directly with any accredited domain registrar, then paste it into the box above:
              </p>
            ...
            {/* Option 2: Done-For-You Concierge */}
            <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/[0.03] p-4 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-bold uppercase tracking-wider text-emerald-700 dark:text-emerald-300 bg-emerald-500/10 px-2 py-0.5 rounded-full flex items-center gap-1">
                  <Sparkles className="h-3 w-3" /> Option B · Done-For-You Concierge
                </span>
                <span className="text-[12px] font-mono font-bold text-emerald-700 dark:text-emerald-300">
                  {currency === "NGN" ? "₦15,000 flat" : "$15 flat"}
                </span>
              </div>
              <p className="text-[13px] text-ink-2 leading-relaxed">
                Not sure how to configure DNS, SPF, DKIM, or MX records? Our specialist team will register your domain and configure 100% of the DNS records for you.
              </p>
      ...
      <Card className="p-0">
        {data.length === 0 ? (
          <div className="py-16 text-center flex flex-col items-center justify-center border-t border-line bg-surface-muted/20">
            ...
            <p className="mt-1 text-[13px] text-ink-3 max-w-sm">Add your first domain to start routing professional mail through Gmail.</p>
          </div>
        ) : (
          <ul className="divide-y divide-line">
            {data.map((d: { id: string; domain_name: string; verification_status: string; created_at: string }) => {
              const verified = d.verification_status === "verified";
              return (
                <li key={d.id} className="group px-5 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="min-w-0">
                    <Link to="/domains/$id" params={{ id: d.id }} className="font-medium hover:underline break-all">
                      {d.domain_name}
                    </Link>
                  ...
                  <div className="flex flex-wrap items-center gap-2 shrink-0 self-start sm:self-auto max-w-full">
                    ...
                    {!verified && (
                      ...
                      >
                        <RefreshCw className={`h-3.5 w-3.5 mr-1.5 ${verifyingId === d.id ? "animate-spin" : ""}`} />
                    ...
                    <button
                      onClick={(e) => { e.preventDefault(); setDeleteError(null); setPendingDelete({ id: d.id, name: d.domain_name }); }}
