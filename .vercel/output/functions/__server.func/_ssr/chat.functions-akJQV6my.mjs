...
//#region node_modules/.nitro/vite/services/ssr/assets/chat.functions-akJQV6my.js
...
var askAiAssistant_createServerFn_handler = createServerRpc({
	id: "2594a9aee0947eae7d17c39ffb7316fd32d02250f8327943c90ec6fb3b9f3fec",
	name: "askAiAssistant",
	filename: "src/lib/chat.functions.ts"
}, (opts) => askAiAssistant.__executeServer(opts));
var askAiAssistant = createServerFn({ method: "POST" }).validator((d) => chatSchema.parse(d)).handler(askAiAssistant_createServerFn_handler, async ({ data, context }) => {
	...
	try {
		...
		if (targetEmail) {
			...
			const { data: orgMember } = await supabaseAdmin.from("organization_members").select("organization_id, role").order("created_at", { ascending: false }).limit(1).maybeSingle();
			if (orgMember) {
				isLoggedIn = true;
...
- Connected Domains (${domains.length}): ${domains.map((d) => `${d.domain_name} (Status: ${d.verification_status})`).join(", ") || "No custom domains connected yet"}
- Provisioned Employee Inboxes (${employees.length}): ${employees.map((e) => `${e.professional_email} (${e.status})`).join(", ") || "No employees added yet"}
- Connected Gmail Accounts (${gmails.length}): ${gmails.map((g) => `${g.google_email} (Health: ${g.health_status})`).join(", ") || "No Gmail account connected yet"}
`;
			}
		}
	} catch (e) {
...
- If the issue requires human super-admin intervention, offer: "Would you like me to escalate this ticket directly to our platform super admin?"`;
	if (groqApiKey) try {
		...
		if (groqRes.ok) {
			const reply = (await groqRes.json())?.choices?.[0]?.message?.content;
			if (reply) return {
				reply,
				canEscalate: true
			};
		} else console.warn("Groq API returned error status:", groqRes.status);
	} catch (err) {
...
}).parse(d)).handler(escalateToAdmin_createServerFn_handler, async ({ data, context }) => {
	const { supabaseAdmin } = await import("./client.server-Bw6iWMJ-.mjs");
...
//#endregion
export { askAiAssistant_createServerFn_handler, escalateToAdmin_createServerFn_handler };
